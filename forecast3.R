...
forw_prices <- read_excel(path = "./data/ffa/panamax/4tc_forw.xlsx",
                             sheet = 1,  # Specify the sheet to read, by name or by number
                             col_names = TRUE,  # Set to FALSE if the first row is not column headers
                             na = "",  # Set the string that represents missing values
                             trim_ws = TRUE)  # Trims leading and trailing whitespace
...
data_combined <- merge(spot_prices, forw_prices, by = "Date")
...
grangerSpot
grangerForw <- causality(var_model, cause = "forwc")
grangerForw
...
for (round in 1:num_rounds) {
  ...
  ## VAR
  lag_order <- VARselect(train_diff_ts, type = "both")$selection["AIC(n)"]
  var_model <- VAR(train_diff_ts, p = 4, type = "both")
  var_fcs <- predict(var_model, n.ahead = forecast_horizon)  
  ...
  var_rev_fcs_forwc <- last_forwc + cumsum(var_fcs$fcst[[2]][, "fcst"])
