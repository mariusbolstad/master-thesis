...
forward_prices <- read_delim('./data/ffa/panamax/pmx_ffa.csv', 
                             delim = ';', 
                             escape_double = FALSE, 
                             col_types = cols(Date = col_date(format = "%d/%m/%Y")),
                             trim_ws = TRUE)
...
# TODO: fix NA values before logging
...
data_log_levels <- data.frame(
  ...
  log_4TC_FORWARD_1Q = log(data_combined$`4TC_P_1Q`),
  log_4TC_FORWARD_2Q = log(data_combined$`4TC_P_2Q`),
  log_4TC_FORWARD_3Q = log(data_combined$`4TC_P_3Q`),
  log_4TC_FORWARD_4Q = log(data_combined$`4TC_P_4Q`),
  log_4TC_FORWARD_1Y = log(data_combined$`4TC_P_1CAL`),
)
