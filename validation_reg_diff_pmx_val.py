...
if local:
    csv_file_spot = "validation/validation_reg_diff_pmx_spot_val.csv"
    csv_file_forw = "validation/validation_reg_diff_pmx_forw_val.csv"
    log = "model_metrics.log"
    max_workers = 2
else:
    csv_file_spot = "/storage/users/mariumbo/validation/validation_reg_diff_pmx_spot_val.csv"
    csv_file_forw = "/storage/users/mariumbo/validation/validation_reg_diff_pmx_forw_val.csv"
...
def mlp_predict(trainX_flat, trainY_flat, nodes, layers, epochs, batch_size, verbose, diff,
    ...
    if earlystop:
        callback = EarlyStopping(monitor='val_loss', patience=earlystop)
        callbacks = [callback]
    ...
    model_mlp.compile(loss='mean_squared_error', optimizer='adam', metrics=['mse'])
...
def main():
    ...
    for dropout in dropout_lst:
        for regul in regul_lst:
            for earlystop in earlystop_lst:
                models = ["MLP", "LSTM", "RW"]
